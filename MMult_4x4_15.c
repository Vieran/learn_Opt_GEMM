/* 优化15
 * 方法：将矩阵C的元素4个一组地进行处理（同一列上4个一组）放到函数中，使用循环融合将4个for循环合并为1个（一次for计算4个元素），并且使用寄存器来存储变量
 * 效果：矩阵小的时候有一点（减免了矩阵B中的每个4xk的重复拷贝）
 * 原因：1. 元素A(0,p)只需要从内存中提取1次而不是4次
 * 		 2. p的每次更新经历了8个浮点运算（提高了计算的密集程度）
 * 		 3. 寄存器存储A和C的相关变量，让变量常驻寄存器，减少caceh miss（不会被驱逐到内存中）
 * 		 4. 使用指针指向B相关的变量，使用ptr++替代B(i,j)涉及的下标计算
 * 		 5. 进一步的循环展开（下表4个一次for计算）
 * 		 6. 指针一次性+4更新（有个特殊的机器指令可以访问ptr+1，而不需要进行ptr++）
 * 		 7. 向量化（__m128d）和矩阵分块（原来只是4个一起算，这里是4x4一起）
 * 		 8. 对C矩阵进行分块（缓解当矩阵增大时性能的下降）
 * 		 9. 计算前对A矩阵进行拷贝
 * 		 10. 计算前对A矩阵进行拷贝，但是去除了重复拷贝
 * 		 11. 计算前对B矩阵进行拷贝
 * 		 12. 计算前对B矩阵进行拷贝，但是去除了重复拷贝
 */
